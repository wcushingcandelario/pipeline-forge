...
# OneNote OpenHierarchy CreateFileType (Microsoft OneNote object model):
# 1 = notebook, 3 = section
$notebookId = ''
$one.OpenHierarchy($root, '', [ref]$notebookId, 1)
...
foreach ($sectionName in $sections.Keys) {
  ...
  $sectionId = ''
  $one.OpenHierarchy($sectionPath, $notebookId, [ref]$sectionId, 3)
